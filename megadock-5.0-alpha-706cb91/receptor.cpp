...
//============================================================================//
template<class T> void Receptor<T>::rpscace(const float &aceratio, const int &num_grid, float *grid_coord,
...
// ##############################
    ...
    for( int l = 0 ; l < na ; l++ ) {// atom-grid assignment
        ...
        const int search_range = (2.4 + this->grid_width -0.01) / this->grid_width;
        const int lc = ng1 * l;          
...
// #################################
    ...
    for( int l = 0 ; l < na ; l++ ) {// atom-grid assignment
        ...
        const int search_range = (12 + this->grid_width -0.01) / this->grid_width;
        const int lc = ng1 * l;
        const int i2 = atom_coord_rotated[3*l  ] / this->grid_width + ng1 / 2;
        const int j2 = atom_coord_rotated[3*l+1] / this->grid_width + ng1 / 2;
        const int k2 = atom_coord_rotated[3*l+2] / this->grid_width + ng1 / 2;
            ...
            for( int j = ja ; j < jb ; j++ ) {
                ...
                for( int k = ka ; k < kb ; k++ ) {
                    ...
                    if( d3 < rjudge2[l]) {// distance(grid-atom) < Van der Waals radius + cutoff D(=3.6ang)
                        ...
                        voxel_rpscace[ijk] += 1.0 + this->_ACE[l] * (-0.8) * aceratio;
...
//============================================================================//
template<class T> void Receptor<T>::electro(const float &beta,const float &eratio,
...
//============================================================================//
{
    ...
    for( int l = 0 ; l < na ; l++ ) {
        const float abs_charge = fabs(this->_Charge[l]);
        ...
        for( int i = ia ; i < ib ; i++ ) {// grid around atom[l]
            ...
            for( int j = ja ; j < jb ; j++ ) {
                ...
                for( int k = ka ; k < kb ; k++ ) {
                    ...
                    const float elec = eratio * beta * this->_Charge[l] / er / d3;
                    grid_i[ijk] += elec;
                }
            }
        }
...
//============================================================================//
template<class T> void Receptor<T>::precalc(const int &num_grid,int *nearesta,float *rjudge2,
...
//============================================================================//
{
    ...
    for( int i = 0 ; i < na ; i++ ) {
        rjudge2[i]      = (this->_Radius[i] + D)*(this->_Radius[i] + D);
...
template class Receptor<ParameterPDB>;
template class Receptor<ParameterTable>;

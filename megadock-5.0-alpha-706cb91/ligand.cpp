...
//============================================================================//
template<class T> void Ligand<T>::rpscace(const float &aceratio, const int &num_grid,float *grid_coord,
...
//============================================================================//
{
    ...
    const int na  = this->num_atoms();
    const int nag = this->num_atoms() * ng1;
    ...
    this->voxel_init(ng1,grid_coord,atom_coord_rotated,grid_r,grid_i,distance3,xd,yd,zd);
...
//   2 is enough because max VdW radius (charmm) is 2.13A and rcore2 = 1.5, so 2.13A * sqrt(1.5) = 2.61A < 3A
    ...
    for( int l = 0 ; l < na ; l++ ) {// atom-grid assignment
        ...
        const int lc = ng1 * l;
        const int i2 = atom_coord_rotated[3*l  ] / this->grid_width + ng1 / 2;
        const int j2 = atom_coord_rotated[3*l+1] / this->grid_width + ng1 / 2;
        const int k2 = atom_coord_rotated[3*l+2] / this->grid_width + ng1 / 2;
...
//============================================================================//
template<class T> void Ligand<T>::electro(const float &beta,const float &eratio,
...
//============================================================================//
{
    ...
    const float pad = (ng1 * this->grid_width / 2);

    for( int l = 0 ; l < na ; l++ ) { //for each atom, find nearest grid(i,j,k)
        //if(fabs(_Charge[l]) < 0.0001) continue;
        const int l3 = l * 3;
        //const int   i = (atom_coord_rotated[l3  ] + pad) / this->grid_width;
        //const int   j = (atom_coord_rotated[l3+1] + pad) / this->grid_width;
        //const int   k = (atom_coord_rotated[l3+2] + pad) / this->grid_width;
        const int   i = max(0, min(ng1 - 1, (int) ((atom_coord_rotated[l3  ] + pad) / this->grid_width)));
        const int   j = max(0, min(ng1 - 1, (int) ((atom_coord_rotated[l3+1] + pad) / this->grid_width)));
        const int   k = max(0, min(ng1 - 1, (int) ((atom_coord_rotated[l3+2] + pad) / this->grid_width)));
        ...
        */
        ...
        grid_i[i*ng2+j*ng1+k] += this->_Charge[l];
    }
...
//============================================================================//
template<class T> void Ligand<T>::precalc(const int &num_grid,int *nearesta,float *rjudge2,
...
//============================================================================//
{
    ...
    for( int i = 0 ; i < na ; i++ ) {
        radius_core2[i] = this->_Radius[i] * this->_Radius[i] * rcore2;
        radius_surf2[i] = this->_Radius[i] * this->_Radius[i] * rsurf2;
    }
...
template class Ligand<ParameterPDB>;
template class Ligand<ParameterTable>;
